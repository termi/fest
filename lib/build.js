...
function help() {
	console.log([
		'Usage:',
		'  fest-build --dir=... [--features=...] [--out=...] [--wrapper=...] [--exclude=...]',
		'',
		'Options:',
		'  --dir       directory where .xml files are',
		'  --features  additional features. The "|" separated list. Possible values: ["string_templates"]',
...
out_dir = out_dir.replace(/[\/\\]$/, '') + path.sep;
if (!IS_WIN && out_dir[0] !== path.sep && process.env.PWD){
	out_dir = process.env.PWD + path.sep + out_dir;
}
...
if (cmd.po) {
	var po_file = cmd.po;
	if (po_file[0] !== path.sep) {
		po_file = process.env.PWD + path.sep + po_file;
	}
	var found_messages = {};
	cmd.compile.events = {
...
cmd.compile.features = (cmd.features || '').split("|");
...
function findfiles(dir, out_dir, prefix){
	...
	files.filter(function(file){
		...
		if(excludeRegexp && excludeRegexp.test(dir + path.sep + file)) {
	...
	if (recursive){
		files.filter(function(file){
			...
			findfiles(dir + file + path.sep, out_dir + file + path.sep, prefix + file + path.sep);
		});
	}
}
...
function mkdir(filename, mode) {
     mode = mode || 0777;
     var parts = filename.split(/[\\/]/);//UNIX and Windows separators
