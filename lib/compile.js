var compile = (function(){
	...
	function escapeRawJS(s){
		return s.replace(jsRawChars, function (chr){
			return jsRawHash[chr];
		});
	}
	...
	function oldCompileAttributes(result, attrIndex, attrName, attrValue, _getExpr){
		var attrExpr;
		result.text += ' ' + attrName + '=\\"';
		attrValue
			.replace(/{{/g, "__DOUBLE_LEFT_CURLY_BRACES__")
			.replace(/}}/g, "__DOUBLE_RIGHT_CURLY_BRACES__")
			.match(/{[^}]*}|[^{}]*/g).forEach(function (str) {
				if (str !== '') {
					if (str[0] === '{') {
						attrExpr = _getExpr(str.slice(1, -1).replace(/__DOUBLE_LEFT_CURLY_BRACES__/g, "{").replace(/__DOUBLE_RIGHT_CURLY_BRACES__/g, "}"), 'expression #' + (attrIndex + 1) + ' in attribute "' + attrName + '"');
						result.expr += (
							'try{__fest_attrs[' + attrIndex + ']=__fest_escapeHTML(' + attrExpr + ')}catch(e){__fest_attrs[' + attrIndex + ']=""; __fest_log_error(e.message);}'
						);
						result.text += '" + __fest_attrs[' + attrIndex + '] + "';
					} else {
	...
	function compileAttributes(options) {
		var attrs = options.attrs,
			parser =options.parser,
			compile_file = options.compile_file,
			_getExpr = options.getExpr,
			canContainsStringTemplates = options.canContainsStringTemplates;

		var attrName, result = { 'text': '', 'expr': '', 'tail': '' }, attrCount = 0, exprCount = 0, attrValue, attrExpr;
		for ( attrName in attrs ) if ( attrs.hasOwnProperty(attrName) ) {
			var parsed = false;
			attrValue = attrs[attrName].value;

			if( canContainsStringTemplates ) {
				attrExpr = compileES6StringTemplate({text: attrValue, attrName: attrName, indexOffset: exprCount, parser: parser, compile_file: compile_file, quote: "\"", isEscapeHTML: true});
				if ( attrExpr.expr ) {
					result.text += (' ' + attrName + '=\\"' + attrExpr.text + '\\"');
					result.expr += attrExpr.expr;
					parsed = true;
					exprCount++;
				}
			}
			...
			if ( !parsed ) {
				oldCompileAttributes(result, attrCount, attrName, attrValue, _getExpr);
				attrCount = result.attrIndex;
			}
		}
	 ...
	 * @example: '$\{1}' -> '${1}', '$\\{2}' -> '$\{2}', '$\\\{2}' -> '$\\{2}'
	 */
	function removeEscapedQuasiPointer(text) {
//		if ( isRaw ) {
//			return text.replace(/\$\\\\{/g, "${");
//		}
		return text.replace(/\$\\\{/g, "$\{").replace(/\$(\\\\)+\{/g, function(a, found){ return "$" + found.substr(-1) + "{" });
	}

	var stringTemplatesMap = {};
	var stringTemplatesCounter = 0;
	function compileES6StringTemplate(options) {
		...
		var resultExpr = '', resultText = '';
		var canContainsQuasiLiterals = text.indexOf("${");

		if ( canContainsQuasiLiterals === -1 ) {//simple text without Quasi Literals
			if ( isEscapeHTML ) {
				text = escapeHTML(text);
			}

			return {expr: null, text: escapeJS(removeEscapedQuasiPointer(text)), tail: null};
		}

		var ast;
		try {
			text = text.replace(/\\\$/g, "\\\\$");// '\${1}' -> '\\${1}' | '\${jsExpr}' is a valid fest string template
			ast = ecmascript6Parser("`" + text + "`", {range: true});
		}
		catch(e) {
			var errorLine = (e + "").match(/Error: Line (\d*)/);
			if ( errorLine && (errorLine = errorLine[1]) != null ) {
				var textLines = text.split("\n");
				errorLine = textLines[(errorLine | 0) - 1];

				if ( errorLine ) {
					errorLine = '. On line "' + errorLine.trim().replace(/\\\\\$/g, "\\$") + '". '
				}
			}
			throw new Error(errorMessage('error on string template parsing' + (errorLine ? '\"' + errorLine + '\" ' : '') + (attrName ? 'on attribute \"' + attrName + '\"' : '') + '. ' + e, parser.line, compile_file));
		}

		try {
			ast = ast.body[0].expression;
			if (ast.type !== "TemplateLiteral") {
				throw new Error("wrong ast tree");
			}
		}
		catch(e) {
			throw new Error(errorMessage('string template is not supported here. ' + (attrName ? ' on attribute \"' + attrName + '\"' : '') + e, parser.line, compile_file));
		}
		...
		if (quasis.length === 0)return {expr: null, text: '', tail: null};
		if (quasis.length === 1) {
			return {expr: null, text: escapeJS(removeEscapedQuasiPointer(text)), tail: null};
		}

		var i = 0, len = quasis.length, expression, exprIndex, quasiRaw;

		if (tagName) {
			var stringTemplate = '';
			var rawStringTemplate = '';
			var funcParams = '';

			while (true) {
				quasiRaw = quasis[i].value.raw;

				quasiRaw = removeEscapedQuasiPointer(quasiRaw);
				if ( isEscapeHTML ) {
					quasiRaw = escapeHTML(quasiRaw);
				}

				stringTemplate += ((i !== 0 ? ',' :'') + quote + escapeJS(quasiRaw) + quote);
				rawStringTemplate += ((i !== 0 ? ',' :'') + quote + escapeRawJS(quasiRaw) + quote);
				if (i + 1 === len) {
					break;
				}
				expression = expressions[i];

				exprIndex = indexOffset + i;
				resultExpr += ('try{__fest_exprs[' + exprIndex + ']=__fest_escapeHTML(' + text.substring(expression.range[0] - 1, expression.range[1] - 1) + ')}catch(e){__fest_exprs[' + exprIndex + ']=""; __fest_log_error(e.message);}');
				funcParams += (',__fest_exprs[' + exprIndex + ']');

				i++;
			}

			var stringTemplatesIndex = stringTemplatesMap[stringTemplate];

			if (stringTemplatesIndex === void 0) {
				if (stringTemplate.length !== rawStringTemplate.length || stringTemplate !== rawStringTemplate) {
					resultExpr += ('__fest_str_templates[' + stringTemplatesCounter + ']=[' + stringTemplate + '];__fest_str_templates[' + stringTemplatesCounter + ']["raw"]=[' + rawStringTemplate + '];');
				}
				else {
					resultExpr += ('__fest_str_templates[' + stringTemplatesCounter + ']=[' + stringTemplate + '];__fest_str_templates[' + stringTemplatesCounter + ']["raw"]=__fest_str_templates[' + stringTemplatesCounter + '];');
				}
				stringTemplatesMap[stringTemplate] = stringTemplatesIndex = stringTemplatesCounter;

				stringTemplatesCounter++;
			}

			resultExpr += ('try{__fest_exprs[' + len + ']=' + tagName + '(__fest_str_templates[' + stringTemplatesIndex + ']' + funcParams + ');}catch(e){__fest_exprs[' + len + ']=""; __fest_log_error(e.message);}');
			resultText = (quote + ' + __fest_exprs[' + len + '] + ' + quote);
		}
		else {
			while (true) {
				quasiRaw = escapeJS(removeEscapedQuasiPointer(quasis[i].value.raw));
				if ( isEscapeHTML ) {
					quasiRaw = escapeHTML(quasiRaw);
				}
				resultText += quasiRaw;
				...
				exprIndex = indexOffset + i;
				resultExpr += ('try{__fest_exprs[' + exprIndex + ']=__fest_escapeHTML(' + text.substring(expression.range[0] - 1, expression.range[1] - 1) + ')}catch(e){__fest_exprs[' + exprIndex + ']=""; __fest_log_error(e.message);}');
				resultText += (quote + ' + __fest_exprs[' + exprIndex + '] + ' + quote);
				i++;
			}
		}

		return {expr: resultExpr, text: resultText, tail: '__fest_exprs.length = 0;'};
	}
	...
	function getExpr(compile_file, parser) {
		return function(value, where) {
			try {
				...
				throw new Error(errorMessage((where || 'node') + ' has ' +  e + '. Node value is "' + value + '"', parser.line, compile_file));
			}
			return value;
		};
	}
	...
	function getEval(compile_file, parser) {
		return function(value) {
			try {
				...
				throw new Error(errorMessage('node has ' + e + '. Node value is "' + value + '"', parser.line, compile_file));
			}
			return value;
		};
	}
	...
	var	festGlobalVarsName = '__fest_global_vars__',
		festGlobalVariables,
		festGlobalVariablesMap;

	function festGlobalVariablesToString() {
		var len = festGlobalVariables.length;
		...
		var head = [], tail = [];
		for( var i = 0 ; i < len ; i++ ) {
			var variable = festGlobalVariables[i]();
			if ( variable.type == "function" ) {
				head.push(variable.declaration);
			}
			else {
				head.push(null);
				tail.push(variable.declaration);
			}
		}
		return festGlobalVarsName + " = [" + head.join(", ") + "];" + tail.join(";");
	}
	...
	function _compile (data){
		...
		var counters = {counter: 0, promises: 1},
			...
			compile_file = translate({
				...
				options: options,
				output: output,
				createVar: createFestGlobalVariables
			}),
			canContainsStringTemplates = options.features.indexOf("string_templates") !== -1,
			_getAttr = getAttr(compile_file, parser),
			_getExpr = getExpr(compile_file, parser),
			_getEval = getEval(compile_file, parser),
			attrs;

		function createFestGlobalVariables(stringValue, type, forceNew) {
			if ( !forceNew && festGlobalVariablesMap[stringValue] !== void 0 ) {
				return festGlobalVariablesMap[stringValue];
			}

			var declaration;
			var index = festGlobalVariables.length;
			var varName = festGlobalVarsName + '[' + index + ']';

			if ( type === 'function' ) {
				declaration = 'function() {try {var res = $1.apply(null, arguments)}catch(e){ return "" };return res}';
			}
			else {
				declaration = 'try {' + varName + ' = $1}catch(e){ ' + varName + ' = "" }';
			}

			festGlobalVariables[index] = function() {
				stringValue = _getExpr("(" + stringValue + ")");
				return {
					type: type,
					declaration: declaration.replace('$1', stringValue),
					name: varName
				}
			};

			if ( !forceNew ) {
				festGlobalVariablesMap[stringValue] = varName;
			}
		...
		parser.onopentag = function (node) {
			...
			if (node.ns[node.prefix] != fest_ns){
				attrs = compileAttributes({attrs: node.attributes, parser: parser, compile_file: compile_file, getExpr: _getExpr, canContainsStringTemplates: canContainsStringTemplates});

				if( attrs.expr ) {
					section.source += attrs.expr;
				}
			...
			switch (node.local){
				...
				case 'text':
					if (node.attributes.tag) {
						templateStringTagStack.push(_getAttr(node, 'tag'));
					}
		...
		parser.onclosetag = function () {
			...
			switch (node.local){
				...
				case 'text':
					if (node.attributes.tag) {
						if (_getAttr(node, 'tag') == templateStringTagStack[templateStringTagStack.length - 1]) {
							templateStringTagStack.pop();
						}
					}
					return;
				case 'if':
		...
		parser.ontext = parser.oncdata = function (text) {
			...
			if (stack[stack.length - 1] === 'script') {
				...
				if (canContainsStringTemplates) {
					attrs = compileES6StringTemplate({text: text, parser: parser, compile_file: compile_file, quote: '"', tagName: templateStringTagStack[templateStringTagStack.length - 1]});
					if( attrs.expr ) {
						section.source += attrs.expr;
					}
					section.source += ('__fest_pushstr(__fest_context,"' + attrs.text + '");');
					if( attrs.tail ) {
						section.source += attrs.tail;
					}
				}
				else {
					// if we are not in es6 mode, just leave Quasi Literals untouched
	...
	function finish_compile(file, options, name){
		...
		var canContainsStringTemplates = options.features.indexOf("string_templates") !== -1;
		if ( canContainsStringTemplates ) {
			if( !ecmascript6Parser ) {
				var exports = {};
				(new Function('exports', readFileSync(__dirname + '/esprima_harmony.js').toString()))(exports);
				ecmascript6Parser = exports.parse;
				exports = null;
			}
		}
		...
		function build_template(output) {
			...
			else if(output.nplurals && typeof options.plural === 'string') {
				options.plural = new Function("n", "return " + options.plural);
			}
			...
			template =
				'function ' + name + '(__fest_context){' +
					...
					'var __fest_self=this,' +
						...
						'__fest_chunk,' +
						'__fest_attrs=[],' +
						(festGlobalVariables.length ? festGlobalVarsName + ',' : '') +
						(canContainsStringTemplates ? '__fest_exprs=[],' : '') +
						(canContainsStringTemplates ? '__fest_str_templates=[],' : '') +
						'__fest_select,' +
						'__fest_if,' +
						'__fest_iterator,' +
						'__fest_to,' +
