function translate(data) {
	...
	PluralMessage.prototype.toString = function() {
		var randomName = "d" + (Math.random() * 1e9 | 0) + Date.now();
		return '' +
			'<fest:script>var ' + randomName + ' = function(n){var res = ' + JSON.stringify(this.plurals) + ';return (res["="+n]||res[__fest_plural(n)]||"").replace("%s",n)}</fest:script>' +
			'<fest:value>' + randomName + '(' + this.select + ')</fest:value>';
	};
	...
	parser.onopentag = function(node){
		...
		if (isI18NNS && node.local === 'plural') {
			...
			output.plural =  true;// TODO:: remove
			output.nplurals = 3;//TODO:: remove, detect it
